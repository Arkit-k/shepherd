...
- ~~**Next:** the **GitHub App**~~ — **DONE (2026-06-30)**, free deterministic tier: server-side PR gate (Check Run + summary comment). See the entry below + `src/app/`.
...
### GitHub App — server-side PR gate (2026-06-30)
The distribution surface from day one's roadmap, shipped as the **free deterministic tier**: the same engine, server-side, gating every PR — no local install, ~$0/PR (no LLM; the deep Claude review stays on the user's own CLI/account). Net-new `src/app/` server, engine reused unchanged.
- **`src/app/server.ts`** — Node built-in `http` server (no Express) hosting Octokit's signature-verifying webhook middleware (`@octokit/webhooks` `createNodeMiddleware(app.webhooks, {path})` — NOT the App-level one, which also mounts OAuth) at `/api/github/webhooks`, plus `/healthz`. Subscribes to `pull_request` opened/synchronize/reopened.
- **`src/app/review.ts`** — the PURE core `reviewCheckout(dir, changedFiles)` (no GitHub calls → unit-testable): runs `scan(dir,{deep:false})`, scopes findings to the changed files (drops whole-repo synthetic findings), `goLiveVerdict`, maps findings → Check annotations (gate→`failure`, advise warn→`warning`/info→`notice`, capped 50), builds a `verdictMarkdown` summary. Plus `handlePullRequest` glue: in-progress check → list changed files → shallow-clone the PR head (token injected into the URL, never logged; fork-safe; branch-clone with sha-fetch fallback) → review → complete check (`success`/`failure`) → upsert the single summary comment (found by a `<!-- shepherd -->` marker) → always clean up the temp clone; errors post a `neutral` check instead of crashing the webhook.
- **`src/app/github.ts`** — thin Checks/PR-files/Comments helpers on an installation Octokit + `installationToken` for the clone. **`src/app/config.ts`** — zod-validated env (`APP_ID`, `PRIVATE_KEY` with `\n` unescaping, `WEBHOOK_SECRET`, `PORT`).
- **Engine reuse, no engine changes** — `scan(deep:false)` is already fully deterministic (its Claude sub-passes self-skip with no `claude` binary), so the free tier falls out for free; it even respects a repo's committed `.shepherd/triage.json`.
- **Ship artifacts** — `src/app/Dockerfile` (multi-stage, `git` at runtime, non-root, healthcheck), `app-manifest.yml` (perms checks:write / pull_requests:write / contents:read; event pull_request), `src/app/README.md` (register → deploy → install; smee.io for local testing). Added `octokit` + `@octokit/webhooks` deps; `npm run app` / `start:app` scripts.
- **Verified** — `reviewCheckout` on a fixture: a changed file with a hardcoded-localhost gate → `conclusion:"failure"`, NOT READY, a `failure` annotation at the right path/line (+ an `any`→`notice`), marker in the summary; a README-only diff → `success` (scoped to 0). Server boots, `/healthz`→200, unknown→404, and an **unsigned webhook→400** (signature verification active). Live end-to-end (open a PR) is the user's step after they register + deploy.
- **Deferred** — paid Claude-deep-review tier (needs billing/quotas), push-to-main trigger, the global cross-user ledger, marketplace listing.

### DevOps scaffolder — generate the infra deck, right-sized (2026-06-30)
Shepherd already KNEW the infra (scale architect prescribed it, the intake captured what the user wants, the probe found the missing security headers + rate limit). `/devops` turns that knowledge into the actual DevOps deck — real, ready-to-adapt config, not prose.
- **`engine/devops-scaffold.ts`** (`devopsBlueprint` + `writeDevopsOrder` → `.shepherd/devops-order.md`) — composes `detectStack` (Node/Next profile, port), the user's `intent.json` (scale + selected infra + deploy target), and the scale-architect prescriptions into tailored config: **GitHub Actions CI/CD** (build → test → `npx shepherd` gate → deploy, with a platform-specific deploy step keyed off the chosen target), **Husky** (pre-commit lint-staged + pre-push tests + `shepherd --git-check`), a **multi-stage Dockerfile** (+ `.dockerignore`, non-root, healthcheck), **docker-compose** standing up the infra the user actually selected (mapped to real service blocks — Valkey/Postgres/Redpanda/Meilisearch/…), a **reverse proxy** (Caddy by default for auto-HTTPS; nginx at k8s/large scale) carrying the exact security headers + rate limit the live probe flags, **Kubernetes** (Deployment/Service/HPA with probes, resource limits, non-root, secretRef), and **Prometheus + Grafana** (scrape config + `prom-client` instrumentation + compose/Helm).
- **Right-sized to the declared scale** (the tweet's lesson applied to ops): `small` → CI + Husky + Docker + Caddy (+ compose if the app already has a datastore); `growing` → + compose with selected infra; `large` → + Kubernetes + Prometheus/Grafana. We don't dump a control plane on a weekend project. Fully deterministic (no Claude, no web — the moat).
- **Ties to real findings** — the proxy config sets the headers the probe reports missing and the rate limit that defends the cost-bomb; the k8s/Docker healthchecks wire to `/healthz`,`/readyz` (the ops check). So it closes findings, not just emits boilerplate.
- **Wired** — `devops` intent + `/devops` (`/cicd`, `/infra-setup`, `/k8s`, `/docker`). **Verified** scale-gating across small/growing/large: small/growing held back k8s + Prometheus; large emitted the full deck and switched Caddy→nginx; selected infra (Redis→Valkey, Meilisearch) appeared as real compose services.
